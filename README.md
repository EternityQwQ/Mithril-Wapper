...
| Metal | **Metal 2**（A11 自带） |
| 系统 | **iOS / iPadOS 13.0** 及以上 |
| MSL | 目标 **MSL 2.2**（iOS 13 对应，MSL 2.3 需 iOS 14 故不使用） |

Metal 后端只使用 Metal 1/2 时代的基础 API（`MTLDevice` / `MTLCommandQueue` /
`MTLRenderPipelineState` / 基础 `MTLPixelFormat`），不依赖 Metal 3 的
indirect command buffer、heap、mesh shader、counter sampling 等新特性，
因此 A11 / Metal 2 设备可完整运行。
